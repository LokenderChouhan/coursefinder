...
const Card = ({ id, review, name, desg, activeId, left }) => {
    return (
      <div
        className="absolute flex-none bg-white drop-shadow-lg p-4 md:p-8 rounded-lg text-left w-[330px] md:w-[370px] h-[320px] md:h-[350px] flex justify-between flex-col transition-position ease-out duration-250"
        style={{
          top: id == activeId ? "30px" : "0",
          left: `${left * 400 + 400}px`
        }}
      >
        <div>
  ...
  return (
    <div className="bg-[#FFDCD8] py-20">
      ...
      <div className="relative w-full p-4 md:p-8 overflow-x-hidden">
        ...
        {/* Desktop 3 card view */}
        <div className="hidden md:flex relative justify-start m-auto gap-[30px] w-[1170px] h-[400px] overflow-x-hidden flex-nowrap">
          {items.map((item, idx) => (
            <Card {...item} activeId={activeId} key={item.id} left={idx-activeId}/>
          ))}
        </div>

        {/* Mobile 1 card view */}
        <div className="flex md:hidden relative justify-center gap-[30px] h-[400px]">
          {items.map((item) =>
            activeId == item.id ? (
